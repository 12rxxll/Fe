...
function revealKnowledgeDetail(){
  const panel=$('knowledgeDetailPanel');
  if(!panel)return;
  setTimeout(()=>{
    const rect=panel.getBoundingClientRect(),small=window.matchMedia&&matchMedia('(max-width: 699px)').matches;
    if(small||rect.top<0||rect.top>window.innerHeight*.82)panel.scrollIntoView({behavior:'smooth',block:'start'});
  },0);
}
...
function bindKnowledgeMapActions(){
  $$('[data-map-term-id]').forEach(button=>{button.onclick=()=>{knowledgeMapMode='tree';selectedKnowledgeTermId=button.dataset.mapTermId;const svc=getKnowledgeService(),term=svc?.getTerm(selectedKnowledgeTermId);if(term)expandKnowledgeCategoryPath(term.categoryId,svc);renderKnowledgeMap();renderKnowledgeDetail(selectedKnowledgeTermId);revealKnowledgeDetail();};button.ondblclick=()=>startKnowledgeTermStudy(button.dataset.mapTermId);});
...
function renderKnowledgeDetail(termId){
  ...
  $$('.map-related-term').forEach(button=>button.onclick=()=>{selectedKnowledgeTermId=button.dataset.mapTermId;renderKnowledgeMap();renderKnowledgeDetail(button.dataset.mapTermId);revealKnowledgeDetail();});
