...
AIへ出した主な指示:
- 基本情報技術者試験の学習アプリであるFE Learning OSに、スマートフォン内蔵センサを使った暗記モードを追加する。
- 既存の学習履歴、記述内容、復習状態、ChatGPT送信履歴、localStorageの保存形式を壊さない。
- 端末を意図的に傾けた時だけ、現在見ている用語の「自分の記述」を一時的に表示し、元の姿勢に戻したら再び隠す。
- iPhone Safariを優先し、DeviceOrientationEvent.requestPermission()が必要な場合に対応する。
- センサ拒否時や非対応時には、ボタン操作だけでも同じ機能を使えるようにする。
- 外部通信、アクセス解析、外部CDN、外部API、カメラ、マイク、位置情報は使わない。
- センサ値の表示だけでなく、可視化、判定、操作として分かるUIを入れる。

AIが提案し、実装に採用した内容:
- スマートフォンの端末方向センサからbetaとgammaを取得し、暗記モード開始時または再調整時の姿勢を基準姿勢にする設計にした。
- 現在のbeta/gammaと基準姿勢との差分から傾き量を計算し、25度以上で表示、10度以内で非表示に戻す判定を入れた。
- 表示開始しきい値と非表示しきい値を分けるヒステリシスを使い、境界付近で答えの表示が点滅しないようにした。
- 傾き量をそのまま使わず、複数回のセンサ値を平均して小さな揺れに反応しにくくした。
- 暗記モード中は用語名と問いは見えるままにし、利用者が自分で書いた記述欄だけを隠すオーバーレイ方式にした。
- センサ状態、基準との差、現在の判定、表示開始しきい値までの残り角度を画面に出し、ゲージで可視化した。
- センサが使えない端末でも課題の動作確認ができるように、長押しで答えを見る操作と、設定によってタップで切り替える操作を用意した。

AIの出力を自分で確認した方法:
- 実際にスマートフォンで暗記モードをONにし、自分の記述だけが隠れ、問いと用語名は表示されたままになることを確認した。
- センサ操作ONで傾き量の表示が変わること、表示開始しきい値を超えた時だけ現在の用語の答えが表示されることを確認した。
- 端末を元の姿勢へ戻した時に答えが再び隠れること、他の用語の記述まで同時に表示されないことを確認した。
- センサを使わない状態でも、長押しボタンまたはタップ切替で答えを表示できることを確認した。
- 設定を変更した後、暗記モード、センサ操作、しきい値、手動表示方式が実際のマップ画面へ反映されるか確認した。
- localStorageの学習履歴や自分の記述が消えないこと、バックアップ対象からセンサの生値が除外されていることを確認した。
