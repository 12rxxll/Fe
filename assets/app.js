...
function weakTerms(system=''){return TERMS.filter(t=>(!system||t['系']===system)&&getTermState(t.id).wrong>0).sort((a,b)=>termScore(b)-termScore(a));}
function unlearnedTerms(system=''){return TERMS.filter(t=>(!system||t['系']===system)&&getTermState(t.id).mastery===0);}
function recommendedTerms(length=10,system=''){const due=dueTerms().filter(t=>!system||t['系']===system).sort((a,b)=>termScore(b)-termScore(a));const weak=weakTerms(system);const fresh=sample(unlearnedTerms(system),Math.max(length,12));const fallback=sample(TERMS.filter(t=>!system||t['系']===system),length);const result=[],seen=new Set();[due,weak,fresh,fallback].forEach(group=>group.forEach(t=>{if(result.length<length&&!seen.has(String(t.id))){seen.add(String(t.id));result.push(t);}}));return result;}
function todayTaskItems(daily,done){const due=dueTerms().length,weak=weakTerms().length,unlearned=unlearnedTerms().length,remaining=Math.max(daily-done,0);const items=[];if(due>0)items.push({label:`復習期限の${due}語を先に戻す`,meta:'忘れかけ対策',kind:'due'});if(weak>0)items.push({label:`誤答が残る${Math.min(weak,10)}語を確認`,meta:'苦手対策',kind:'weak'});if(remaining>0)items.push({label:`今日の目標まであと${remaining}問`,meta:'おすすめ演習',kind:'quiz'});if(unlearned>0)items.push({label:`未学習語を少し追加`,meta:`残り${unlearned}語`,kind:'new'});return items.slice(0,4);}
function quizWeakSystems(results){const wrong=results.filter(r=>!r.correct).map(r=>termById.get(String(r.id))).filter(Boolean);const counts={};wrong.forEach(t=>{counts[t['系']]=(counts[t['系']]||0)+1;});return Object.entries(counts).sort((a,b)=>b[1]-a[1]);}
...
function setup(){
  ...
  $('recommendedQuizButton').addEventListener('click',()=>{navTo('quiz');$('quizMode').value='recommended';$('quizLength').value='10';startQuiz({mode:'recommended',length:10});});
...
function startQuiz(overrides={}){const mode=overrides.mode||$('quizMode').value,system=overrides.system??$('quizSystem').value,length=Number(overrides.length||$('quizLength').value),type=overrides.type||$('quizType').value,pool=selectPool(mode,system,length);const chosen=mode==='recommended'?pool.slice(0,Math.min(length,pool.length)):sample(pool,Math.min(length,pool.length));quiz={mode,system,type,length:chosen.length,index:0,correct:0,answered:false,items:chosen.map(t=>makeQuestion(t,type)),results:[],startedAt:Date.now()};$('quizSetup').classList.add('hidden');$('quizArea').classList.remove('hidden');renderQuizQuestion();}
