...
function emptyTermState(){return {mastery:0,correct:0,wrong:0,streak:0,ease:2.5,interval:0,repetitions:0,due:null,nextReview:null,last:null,lastCorrect:null,lastWrong:null,learnedAt:null,updatedAt:null,reviewPriority:0,bookmark:false,note:''};}
function migrateTermState(value={}){const s=Object.assign(emptyTermState(),value||{});s.mastery=clamp(Number(s.mastery)||0,0,5);s.correct=Math.max(0,Number(s.correct)||0);s.wrong=Math.max(0,Number(s.wrong)||0);s.streak=Math.max(0,Number(s.streak)||0);s.ease=Math.max(1.3,Number(s.ease)||2.5);s.interval=Math.max(0,Number(s.interval)||0);s.repetitions=Math.max(0,Number(s.repetitions)||0);s.due=s.due||s.nextReview||null;s.nextReview=s.nextReview||s.due||null;s.last=s.last||s.lastCorrect||s.lastWrong||null;s.reviewPriority=Math.max(0,Number(s.reviewPriority)||0);return s;}
...
function termStateNeedsMigration(s){return !('nextReview' in s)||!('lastCorrect' in s)||!('lastWrong' in s)||!('reviewPriority' in s);}
...
function stackTerm(t){const s=readTermState(t.id);return `<button class="stack-item term-link text-button" data-term-id="${esc(t.id)}"><span class="stack-item-main"><strong>${esc(t['用語'])}</strong><small>${esc(t['系'])}・正解${s.correct}/誤答${s.wrong}・次回 ${esc(formatReviewDate(reviewDate(s)))}</small></span><b>Lv.${s.mastery}</b></button>`;}
...
function answerSubjectQuestion(id,button){if(!quiz||quiz.answered)return;quiz.answered=true;const q=quiz.items[quiz.index],target=q.target,correct=Number(id)===Number(target.answer),today=localDate(),s=getSubjectState(target.id);if(correct){quiz.correct++;s.correct++;s.lastCorrect=today;}else{s.wrong++;s.lastWrong=today;}s.last=today;state.subjectA.attempts.push({id:target.id,correct,date:today,ts:Date.now(),mode:quiz.mode,system:target.system,category:target.category});quiz.results.push({id:target.id,correct,source:'subjectA',system:target.system});$$('[data-answer-id]').forEach(b=>{b.disabled=true;if(Number(b.dataset.answerId)===Number(target.answer))b.classList.add('correct');});if(!correct)button.classList.add('wrong');saveState(false);$('quizFeedback').innerHTML=`<div class="explanation"><strong>${correct?'正解':'不正解'}</strong><p><b>正解：</b>${esc(target.choices[target.answer])}</p><div class="explanation-block"><h4>解説</h4><p>${esc(target.explanation)}</p></div><div class="explanation-block"><h4>関連語</h4><div class="badges">${target.relatedTerms.map(x=>`<span class="badge gray">${esc(x)}</span>`).join('')}</div></div><div class="quiz-actions"><button id="nextQuestionButton" class="primary" type="button">${quiz.index+1>=quiz.length?'結果を見る':'次の問題'}</button></div></div>`;$('nextQuestionButton').onclick=()=>{quiz.index++;renderQuizQuestion();};}
function answerQuestion(id,button){if(!quiz||quiz.answered)return;const q=quiz.items[quiz.index];if(q.source==='subjectA')return answerSubjectQuestion(id,button);quiz.answered=true;const correct=String(id)===String(q.target.id),today=localDate();if(correct)quiz.correct++;const s=getTermState(q.target.id);s.last=today;s.learnedAt=s.learnedAt||today;s.updatedAt=today;if(correct){s.correct++;s.streak++;s.lastCorrect=today;s.mastery=Math.max(s.mastery,2);}else{s.wrong++;s.streak=0;s.lastWrong=today;s.mastery=Math.min(Math.max(s.mastery,1),2);}state.attempts.push({id:String(q.target.id),correct,date:today,ts:Date.now(),mode:quiz.mode,type:q.qtype,system:q.target['系']});quiz.results.push({id:q.target.id,correct,source:'terms',system:q.target['系']});$$('[data-answer-id]').forEach(b=>{b.disabled=true;if(String(b.dataset.answerId)===String(q.target.id))b.classList.add('correct');});if(!correct)button.classList.add('wrong');scheduleReview(q.target.id,correct?4:1);saveState(false);$('quizFeedback').innerHTML=`<div class="explanation"><strong>${correct?'正解':'不正解'}</strong><p><b>正解：</b>${esc(q.target['用語'])}</p>${answerExplanation(q,id,correct)}<div class="rating-row"><button data-rating="1">もう一度</button><button data-rating="3">難しい</button><button data-rating="4">理解</button><button data-rating="5">簡単</button></div><div class="quiz-actions"><button id="nextQuestionButton" class="primary" type="button">${quiz.index+1>=quiz.length?'結果を見る':'次の問題'}</button></div></div>`;$$('[data-rating]').forEach(b=>b.onclick=()=>{scheduleReview(q.target.id,Number(b.dataset.rating));saveState(false);$$('[data-rating]').forEach(x=>x.disabled=true);showToast('復習間隔を更新しました');});$('nextQuestionButton').onclick=()=>{quiz.index++;renderQuizQuestion();};}
