...
この記録は、AIに何を指示し、AIがどのような実装案を出し、それを自分でどう確認・修正したかを説明するためのもの。

使用した生成AI:
ChatGPT / Codex

AIへ出した主な指示:
- 基本情報技術者試験の学習アプリであるFE Learning OSに、スマートフォン内蔵センサを使った「暗記モード」を追加するように指示した。
- 暗記モード開始時は、全用語の「自分の記述」を隠し、用語名と問いだけを見て思い出せる状態にするよう指示した。
- 端末を意図的に傾けた時だけ、現在見ている用語の記述を一時的に表示し、元の姿勢に戻したら再び隠すように指示した。
- 次の用語へ移動しても暗記モード中は隠れた状態を維持し、前の用語の表示状態を引き継がないように指示した。
- 端末方向センサが使えない場合や許可されない場合でも、長押しまたはタップ切替の手動操作で同じ学習ができるように指示した。
- iPhone Safariを優先し、DeviceOrientationEvent.requestPermission()が必要な場合はユーザー操作中に許可要求するように指示した。
- 暗記モード、センサ操作、表示しきい値、非表示しきい値、維持時間、手動表示方式を設定画面と連動させるように指示した。
- 暗記モードは用語カードごとの機能ではなく、マップ画面全体の共通機能に整理するように指示した。
- 長押し操作がマップ上部の共通ボタンだけでなく、各用語の記述欄の下でも使えるようにするよう指示した。
- 外部通信、アクセス解析、外部CDN、外部API、カメラ、マイク、位置情報は使わず、センサ値も保存・送信しないように指定した。
- 課題提出で説明しやすいように、センサ値の表示だけでなく、可視化、判定、操作として分かるUIを入れるように指示した。

AIが提案し、実装に採用した内容:
- 暗記モードの設定値はlocalStorageに保存し、センサの生値、基準姿勢、履歴はメモリ上だけで扱う設計にした。
- DeviceOrientationEventからbetaとgammaを取得し、暗記モード開始時または「センサ再調整」時の姿勢を基準姿勢にする構成を採用した。
- 現在のbeta/gammaと基準姿勢との差分から傾き量を計算し、約25度以上で表示、約10度以内で非表示へ戻す判定を入れた。
- 表示開始しきい値と非表示しきい値を分けるヒステリシスを使い、境界付近で答えの表示が点滅しないようにした。
- 直近のセンサ値を平均して小さな揺れをならし、少し持ち替えただけでは答えが出ないようにした。
- 暗記モード中は用語名と問いを表示したまま、textareaの上にオーバーレイを置き、記述欄の高さを保ったまま内容だけを隠す方式にした。
- 傾けた時に表示する対象は、現在開いている用語、入力中の用語、画面中央に近い用語の順で判断する設計にした。
- マップ画面上部に、暗記モード、センサ操作、センサ再調整、すべて表示、現在の答えを見る共通操作をまとめた。
- センサ状態、基準との差、現在の判定、表示開始しきい値までの残り角度を画面に出し、ゲージで可視化した。
- センサ操作がOFFの時は、各用語の記述欄の下に「長押しで表示」または「タップで表示」ボタンを出すようにした。
- 設定画面の手動表示方式を反映し、長押し設定では押している間だけ、タップ切替設定ではタップで表示/非表示を切り替えるようにした。
- PWAで古いJavaScriptやCSSが残らないよう、アプリのバージョンとService Workerのキャッシュ名を更新する構成にした。

AIの出力を自分で確認した方法:
- まず既存のマップ画面、用語カード、記述欄、設定保存処理を確認し、既存の記述内容や学習履歴を消す変更になっていないか確認した。
- 暗記モードをONにした状態で、自分の記述だけが隠れ、問いと用語名は表示されたままになることを確認した。
- センサ操作ONで傾き量の表示が変わること、表示開始しきい値を超えた時だけ現在の用語の答えが表示されることを確認した。
- 端末を元の姿勢へ戻した時に答えが再び隠れること、他の用語の記述まで同時に表示されないことを確認した。
- センサ操作OFF時に、各用語カードの記述欄下へ手動表示ボタンが出ることを確認した。
- 長押し方式では押している間だけ答えが表示され、指を離すと隠れることを確認対象にした。
- タップ切替方式ではボタンの表示文言が切り替わり、設定した操作方法が画面に反映されることを確認対象にした。
- 設定を変更した後、暗記モード、センサ操作、しきい値、手動表示方式、センサパネル表示が実際のマップ画面へ反映されるか確認した。
- 320px、375px、390px、430pxのスマホ幅で横スクロールや表示崩れが出ないか確認した。
- ブラウザのコンソールエラーが出ていないか確認した。
- GitHub Pagesで新しいバージョンが返ること、Service Workerのキャッシュ名が更新されていることを確認した。
- localStorageの学習履歴や自分の記述が消えないこと、センサの生値が保存対象に含まれないことを確認した。

自分で確認・修正した点:
- 最初は暗記モードやセンサ操作が用語カード内に入りすぎていたため、用語ごとの機能ではなくマップ全体の共通操作として整理し直した。
- iPhone Safariではユーザー操作中でないとDeviceOrientationEvent.requestPermission()を呼べないため、センサ操作ONボタンを押した時に許可確認する流れへ修正した。
- 暗記モードがOFFなのにセンサ操作だけONにできると動作が分かりにくいため、暗記モードOFF時はセンサ操作を無効化し、内部状態もOFFへ戻すようにした。
- センサ許可を拒否した場合や非対応ブラウザの場合にアプリが止まらないよう、ボタン操作のみへ切り替える表示と処理を追加した。
- 長押しで答えを見る機能がマップ上部の暗記モード設定内でしか使えない状態になっていたため、各用語の記述欄の下にも専用ボタンを出すように修正した。
- 用語カード全体を長押し対象にすると誤操作が起きやすいため、枠自体をボタンにせず、明確な「長押しで表示」ボタンだけを操作対象にした。
- センサ操作ON時は傾き操作を優先し、センサ操作OFF時だけ各用語の手動表示ボタンが出るようにして、操作方法が混ざらないようにした。
- 設定で「タップで表示切替」を選んだ場合は、ボタン文言と動作が長押しではなくタップ切替になるように修正した。
- 長押しで答えを見る時、iOSの文字選択やルーペが保存状態の文字に反応して邪魔になったため、保存状態の表示を選択不可にした。
- 状態を色だけで表すと分かりにくいため、「センサON」「ボタン操作のみ」「答えを隠しています」などの文字ラベルも表示した。
- PWAでは古いJavaScriptやCSSがキャッシュに残ることがあったため、変更のたびにバージョン番号とService Workerのキャッシュ名を更新した。

課題要件との対応:
- 生成AIの使用: ChatGPT/Codexに要件整理、実装案、コード修正、確認観点の整理を依頼した。
- スマートフォン内蔵センサ: DeviceOrientationEventで端末方向センサのbeta/gammaを利用した。
- センサ値の表示: センサ状態、基準姿勢との差、表示までの残り角度を画面に表示した。
- 可視化・判定・操作: 傾き量をゲージで可視化し、しきい値を超えた時だけ答えを表示する判定と操作に使った。
- 安全性: センサ値を保存・送信せず、カメラ、マイク、位置情報、外部通信、アクセス解析を使わない設計にした。
- 動作確認: iPhone SafariまたはPWAとして動作確認し、センサ許可やHTTPSが必要な点を問題点として記録した。

Codex共有リンクについて:
Codexの codex://threads/... はアプリ内でスレッドを開くための内部リンクで、採点者が外部から閲覧できる公開共有リンクではない。
また、この開発スレッドには課題以外の作業、GitHub操作、個人情報に近い内容も混ざるため、そのまま共有すると安全性と分かりやすさの両方に問題がある。
